...
void getsym(void){
	...
	if (isalpha(ch)){ //当前输入为字母，则输入词法符号应该为关键字或标识符
		...
		do{
			if (k < MAXIDLEN)
				a[k++] = ch;
			getch(); // 循环结束时，多读取了一个字符
		} while (isalpha(ch) || isdigit(ch));//字母开头的字母数字组合，巧妙设置实现了“以字母开头，字母和数字组成”的标识符规则
		a[k] = 0;//添加字符串结尾符
		strcpy(id, a);
		...
		if (++i) { // i + 1 != 0 即 i != -1, 该单词是保留字
			sym = wsym[i]; // symbol is a reserved word
			// printf("{ i=%d, wsymp[i]=%d }\n", i, wsym[i]);
		} else
			sym = SYM_IDENTIFIER;   // symbol is an identifier 变量/常量名/过程名
	}else if (isdigit(ch)) { // symbol is a number.
		k = num = 0;
		sym = SYM_NUMBER;
		do { //识别数字
			num = num * 10 + ch - '0'; // 将该数字字符转换成数值，并继续并入后续数字字符
			k++;
			getch();
		} while (isdigit(ch));
		if (k > MAXNUMLEN)
	...
	do{
		if (sym == SYM_CONST){ // constant declarations
			...
			do{//循环处理id1=num1,id2=num2,……
				...
				while (sym == SYM_COMMA){ // 上一组常量定义代码段（上一行中的函数内部逻辑）之后的第一个字符是','
					getsym();
					constdeclaration();
				}// const a=1,b=2; a=3;
				//捕获到';'，认为常量定义结束
				if (sym == SYM_SEMICOLON){ // 获取到分号';'      打断在此（12.12）
					getsym(); // 继续获取下一个字段
				}else{
					error(5); // Missing ',' or ';'.
				}
			}while (sym == SYM_IDENTIFIER);
			// printf("Just After Const...");
			// exit(0);
		} // if
...
//			block = dx;
		...
		// test(set, statbegsys, 7);
		destroyset(set1);
		destroyset(set);
		// if (sym == SYM_BEGIN) {
		// 	printf("begin in declbegsys = %d\n", inset(sym, declbegsys));
		// 	printf("begin in statbegsys = %d\n", inset(sym, statbegsys));
		// 	exit(0);
		// }
